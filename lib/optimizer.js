    ...
    if( Array.isArray( mod ) ) {
        if( mod.length > 0 ) {
            return Promise.map( mod, function(dep) {
                if( config.paths[dep] ) {
                    return resolve( config.paths[dep], config, dep );
        ...
        return load( filepath, readOnly ).then( function(source) {
            ...
            return resolve( externalDependencies, config ).then( function(resolvedDeps) {
                return {
                    deps:       resolvedDeps,
                    src:        source,
...
function build(result, config, pgraph) {
    ...
    var finish = co.wrap( function*() {
        ...
        if( config.deps ) {
            let deps = yield resolve( config.deps, config );
