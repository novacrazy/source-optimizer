...
var stripRelativeRe = /(\.\/)?([\w\-\/]*)(\.\w+)?/g; //Use $2
var stripExtrasRe = /\.\/([\w\-\/]*)\.\w+/g; //Use $1
var firstDefineRe = /define\s*\(\s*(?!['"])/;
var otherDefineRe = /define\s*\(\s*["']([\w\/]*)['"]/g; //Use $1
var defineDepsRe = /(define\s*\(\s*.*?)\[(.*?)\]/g; //Use $1 for module, $2 for deps
var shimWindowRe = /window\.(\w+)\s*=/; //use $1
...
function resolve(mod, config, loader, name, readOnly) {
    ...
    var dir = path.dirname( mod );
    var url = path.normalize( pathify( baseUrl, mod ) );

    var resolveFromSource = function(source) {
        var moduleName = name || path.relative( baseUrl, mod ).replace( stripRelativeRe, '$2' );

        var internalModules = [];
        var externalDependencies = [];

        var anonymousDefine = false;
