...
function resolve(mod, config, name, load) {
    config = normalize( config );

    let baseUrl = config.baseUrl;

    if( Array.isArray( mod ) ) {
        let dir = name; //when resolving an array, name is the directory

        if( mod.length > 0 ) {
            return Promise.map( mod, function(dep) {
                if( config.paths[dep] ) {
                    return resolve( config.paths[dep], config, dep, load );

                } else {
                    return resolve( path.normalize( pathify( dir, dep ) ), config, null, load );
                }
            } );
        ...
        let filepath = config.paths[moduleName] ? path.resolve( mod ) : url;

        return load( filepath, readOnly ).then( function(source) {
            let internalModules = [];
...
function build(result, config, pgraph) {
    ...
    var finish = co.wrap( function*() {
        ...
        if( config.deps ) {
            let deps = yield resolve( config.deps, config, null, result.load );

            for( let it in deps ) {
                if( deps.hasOwnProperty( it ) ) {
                    resultingSource += yield build( deps[it], config, graph );
                }
            }
        }
...
function pathify(/*...*/) {
    return _( arguments ).filter( null ).reduce( function(acc, t) {
